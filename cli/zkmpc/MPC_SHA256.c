...
int main(int argc, char * argv[]) {
	...
	if ((argc != 3) && (argc != 2))
	{
		printf("Usage: %s <secret> [<challenge>]\n",argv[0]);
		return -1;
	}
	...
	for(int k=0; k<NUM_ROUNDS; k++) {
		for(int j = 0; j<3; j++) {
			...
			getAllRandomness(keys[k][j], randomness[k][j], challenge);
		}
	}
	...
	printf("myquantumkey value of secret is:");
	for (int i = 0; i < 8; i++)
	{
		temphash[i*sizeof(int)] = (finalHash[i]>>24)&0xFF;
		temphash[i*sizeof(int)+1] = (finalHash[i]>>16)&0xFF;
		temphash[i*sizeof(int)+2] = (finalHash[i]>>8)&0xFF;
		temphash[i*sizeof(int)+3] = (finalHash[i])&0xFF;
	}
	...
	sprintf(outputFile, "%s_%i.zkmpc",challenge,NUM_ROUNDS);
